/* Path: src/app/admin/add-achievement/page.js */
/* Modifikasi: Menambahkan input 'year' */
...
import { Upload, Loader2, Save, ArrowLeft } from 'lucide-react'; // Menghapus AlertTriangle yang tidak digunakan
...
export default function AddAchievementPage() {
  ...
  const [year, setYear] = useState(new Date().getFullYear()); // <-- State baru, default tahun ini
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!file) {
      setMessage({ type: 'error', text: 'Foto wajib di-upload.' });
      return;
    }
    ...
    try {
      const resApi = await fetch('/api/achievements', {
        method: 'POST',
      ...
      setMessage({ type: 'success', text: 'Achievement berhasil ditambahkan!' });
      
      // Arahkan kembali ke halaman kelola
      setTimeout(() => {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <h1 className="text-3xl font-bold text-gray-800 mb-6">Tambah Achievement Baru</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-md space-y-6">
        ...
        {/* --- [MODIFIKASI] Judul & Tahun dalam 1 baris --- */}
        ...
        <div className="text-right">
          ...
          >
            ...
            {saving ? 'Menyimpan...' : 'Simpan Achievement'}
          </button>
        </div>
      </form>
    </div>
